...
def merge_scan(existing: dict, entries: list[dict]) -> dict:
    result: dict[str, dict] = {
        key: {field: record[field] for field in SHARED_PLUGIN_FIELDS if field in record}
    ...
    for entry in entries:
        ...
        for field in ("key", "kind", "rel", "id", "name", "folder_name", "version"):
            if field in entry and (entry[field] not in (None, "") or field in {"key", "kind", "rel"}):
